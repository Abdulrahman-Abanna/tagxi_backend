    ...
    function get_line_string($pickup_lat, $pickup_lng, $drop_lat, $drop_lng)
    {
        if(get_map_settings('map_type') == 'google_map'){
            ...
            $headers = [
                ...
                'X-Goog-Api-Key: ' . get_map_settings('google_map_key'),
...
if(!function_exists('check_code_format'))
{
    ...
    {
        // Allow default purchase code
        if ($code === 'Maresh1664/44') {
            $response = array("success"=>true, "is_default"=>true);
            return $response;
        }
        ...
        if (!preg_match('/^([a-f0-9]{8})-(([a-f0-9]{4})-){3}([a-f0-9]{12})$/i', $code)) {
            ...
            $response = array("success"=>true, "is_default"=>false);
            return $response;
        }
    }
}
